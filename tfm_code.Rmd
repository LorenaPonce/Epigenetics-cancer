...
## PROBES DIFERENCIALES ENTRE LOS DISTINTOS GRUPOS DE ESTUDIO: tumoral-normal + cohortes
...
ntCOAD <- topTable (efit, number=nrow(efit), coef="COADsamples", adjust="fdr")#, genelist=ann450kSub
ntBRCA <- topTable (efit, number=nrow(efit), coef="BRCAsamples", adjust="fdr")
ntPRAD <- topTable (efit, number=nrow(efit), coef="PRADsamples", adjust="fdr")
ntLUAD <- topTable (efit, number=nrow(efit), coef="LUADsamples", adjust="fdr")
...
class <- c(rep("tumor-COAD", 30), rep("tumor-BRCA", 49), rep("tumor-PRAD", 50),  rep("tumor-LUAD", 25))


MvalF_t <- MvalF[, grep("01A", colnames(MvalF))]
...
MvalF_t <- t(MvalF_t)
MvalF_t <- transposeBigData(MvalF_t)
```


```{r}
#mds <- cmdscale(dist(scapulae[,-8]),eig=T
```
