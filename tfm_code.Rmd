...
(imallona: replacing the two dimensional apply by a one-dimensional one, by row, to speed it up)
...
Mval.coad <- apply(assay(met.coad), 2, beta2m)
Mval.brca <- apply(assay(met.brca), 2, beta2m)
Mval.prad <- apply(assay(met.prad), 2, beta2m)
Mval.luad <- apply(assay(met.luad), 2, beta2m)
...
(imallona commented out because)
```label: ttest.abc.luad
Quitting from lines 1069-1073 (tfm_code.Rmd)
Error in if (stderr < 10 * .Machine$double.eps * max(abs(mx), abs(my))) stop("data are essentially constant") :
  missing value where TRUE/FALSE needed
```

```{r ttest.abc.luad, eval = FALSE}
...
for(i in 1:4){
  ...
  pdf(file=file.path("outputs", file), paper="special", width=6, height=6)
...
#### Extra added differential methylation proposal

(Izaskun) Feel free to discard. My main aim is to first work with M values, and second add the patient effect with a nested factorial design.

We could even nest this further: the `disease` can be specified as an extra column of the metadata matrix if we wanted to run the analysis with all `coad`+`luad`+`prad`+`brca` `assay`s together.


```{r diff_meth_alternative}

ids_coad <- data.frame(patient  = as.factor(strtrim(colnames(assay(met.coad)), 12)),
                       status = sapply(strsplit(colnames(assay(met.coad)), '-'),
                                       function(x) return(strtrim(x[4], 2))),
                       disease = 'coad')

rownames(ids_coad) <- colnames(assay(met.coad))

## makeContrasts is picky with `-` as levels
ids_coad$patient <- make.names(as.factor(gsub( '-', '_', ids_coad$patient )))
table(ids_coad)

design_alt <- model.matrix(  ~ 0+ status + status:patient, ids_coad)

fit_alt <- lmFit(apply(assay(met.coad), 2, beta2m), design_alt)

colnames(design_alt)

## this breaks here as the make.names is not resolving the odd TCGA naming, @todo @imallona fix

## contrasts_alt <- makeContrasts(tumorvsnormal = status11 - status01,
##                                     levels = make.names(colnames(design_alt)))

## fit2_alt <- contrasts.fit(fit_alt, contrasts_alt)
## efit_alt <- eBayes(fit2_alt)
## head(topTable(efit_alt))
...
## building an index with whether normal or tumor, and the patient id
## with first the old data
idx_met_coad <- data.frame(id = colnames(assay(met.coad)),
                           status = sapply(strsplit(colnames(assay(met.coad)), '-'),
                                           function(x) return(strtrim(x[4], 2))),
                           patient = strtrim(colnames(assay(met.coad)), 12),
                           stringsAsFactors = FALSE)

## and second the latest data
idx_mval_coad <- data.frame(id = colnames(Mval.coad),
                            status = sapply(strsplit(colnames(Mval.coad), '-'),
                                            function(x) return(strtrim(x[4], 2))),
                            patient = strtrim(colnames(Mval.coad), 12),
                            stringsAsFactors = FALSE)

table(idx_met_coad$patient)
table(idx_mval_coad$patient)

## they are the same, let's get the old data and directly subtract,
## as the samples are beautifully ordered already
all(idx_met_coad[idx_met_coad$status == '01', 'patient'] ==
    idx_met_coad[idx_met_coad$status == '11', 'patient'])

## so to get delta tumor vs normal it's subtracting the matrix of statuses `01` to
##  those with stuses `11`
suggested_delta <- assay(met.coad)[,idx_met_coad[idx_met_coad$status == '01', 'id']] -
    assay(met.coad)[,idx_met_coad[idx_met_coad$status == '11', 'id']]

## trivial tests on the first element, first cg
assay(met.coad)[1,'TCGA-AA-3713-01A-21D-1721-05'] -
    assay(met.coad)[1, 'TCGA-AA-3713-11A-01D-1721-05'] == suggested_delta[1,1]

## and the third cpg, this patient
assay(met.coad)[10,'TCGA-AZ-6599-01A-11D-1772-05'] -
    assay(met.coad)[10, 'TCGA-AZ-6599-11A-01D-1772-05'] ==
    suggested_delta[10,'TCGA-AZ-6599-01A-11D-1772-05']

## still is dirty to have the suggested_delta colnames as if they were tumors,
##  let's rename it
colnames(suggested_delta) <- sprintf('%s_delta_tn', strtrim(colnames(suggested_delta), 12))
